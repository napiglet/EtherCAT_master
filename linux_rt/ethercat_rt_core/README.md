...
- Distance-based 7-segment jerk-limited S-curve planner for position moves.
...
CiA402 commands through the same API used by the mock backend. MoveAbs/MoveRel
are generated by the backend as internal CSP motion profiles. `Trapezoidal`
uses a linear acceleration profile, while `SCurve`, `JerkRatio`, and `LMS`
use the distance-based 7-segment jerk-limited planner documented in
`docs/MOTION_PROFILE_7SEGMENT_SCURVE_KR.md`. Jog/MoveVel use CSV velocity
ramping.
